...
df_raw=read.csv(paste0(root.dir,"Datasets_Raw/Bancarrota3years.csv"), row.names=1, na.strings="?")
```
...
Mirando en la siguiente gráfica, podemos ver que la variable con mayor cantidad de valores faltantes es el atributo 37, que de hecho tiene 4736 valores faltantes (aproximadamente la mitad de los NAs). Sabiendo que el dataset tiene 10503 datos, esto significa que aproximadamente la mitad de sus valores son faltantes.
...
Para los atributos 28,53 y 54, como los valores negativos no se van tantas escalas hacia abajo, los limitaremos a -10; mientras que los valores positivos les aplicaremos log(1+x). Esto último también se lo haremos al atributo 64.
...
Mirando la distribución a continuación, podemos ver que la mayor parte de los valores están entre 0-0.3, y si nos vamos a la siguiente escala no quedan apenas valores; es decir, fuera de ]-3,3[ no hay apenas valores. 
...
```
Por este motivo, para esta variable simplemente acotaremos los valores a -3,3 e imputaremos después con kNN.
```{r}
...
saveRDS(df_raw,paste0(root.dir,"Datasets_procesados/1_Procesado_outliers_NAs.Rds"))
```

## Selección de variables
 
A lo largo de esta sección elegiremos las variables mas importantes de nuestro dataset para su uso en modelos. Así, el objetivo es reducir todo lo posible el número de atributos para simplificar el modelo final. Esto lo haremos en 2 pasos: primero, miraremos variables altamente correlacionadas y eliminaremos algunas de ellas (para evitar problemas con modelos como lda, glm...)

## Correlación entre variables

```{r}
df_raw=readRDS(paste0(root.dir,"Datasets_procesados/1_Procesado_outliers_NAs.Rds"))
```

A lo largo de esta sección analizaremos la correlación que hay entre las variables y eliminaremos las variables mas correladas entre sí. Se hará así una selección para dismininuir el número de variables altamente correladas entre sí.

```{r}
library(correlation)
library(corrplot)

corr_matrix <- cor(df_raw)
...
-   28,53 y 54. Estas tres variables se procesaron a la vez; en este procesamiento también estaba el Attr64 pues las 4 variables tienen el mismo denominador, pero el Attr64 era la única que siempre era positiva, y por lo que se ve no está tan correlada, pero si lo está bastante (correlación 0.5-0.6 con cada una de las variables). Por estos motivos, eliminaremos dos de ellas, en particular, nos quedaremos con el Attr54 que es el que mas correlación tiene en común con las otras 2.

```{r}
corrplot(cor(df_raw[,paste0("Attr",c(28,53,54,64))]), method = "circle", addCoef.col = "black")
df_raw=subset(df_raw,select=-c(Attr28,Attr53))
```

-   2,3,10,25,38 y 51. Estas variables fueron preprocesadas a la vez en el tratamiento de outliers restantes, aplicándoles transformaciones genéricas. Su alta correlación es probablemente debido a que comparten el denominador "activo total". Eliminaremos varias variables, para empezar, el atributo2 y el atributo 10, son prácticamente iguales salvo que el atributo 2 es positivo, por lo que por preferencia mía, nos quedaremos con el 2 (prefiero números que se que son positivo). Este parecido se extiende, aunque de forma menor a los atributos 38 y 51 (que entre ellos tienen una correlación de 0.98!!), por lo que estos dos atributos también los eliminaremos. Por tanto, de momento nos quedaremos con los atributos 2,3 y 25.

```{r}
corrplot(cor(df_raw[,paste0("Attr",c(2,3,10,25,38,51))]), method = "circle", addCoef.col = "black")
df_raw=subset(df_raw,select=-c(Attr10,Attr38,Attr51))
```

-   32,33,52 y 63. No es sorprendente que estén todos altamente relacionados, pues usan en sus numeradores o denominadores variables de pasivos o coste de productos. Curiosamente, por los denominadores, se tratan simultáneamente el 32 y el 52, y luego el 33 y el 63.

```{r}
corrplot(cor(df_raw[,paste0("Attr",c(32,33,52,63))]), method = "circle", addCoef.col = "black")
```

-   4,8,17,40,46 y 50. Todos estos atributos los tratamos a la vez, y tuvieron exactamente las mismas transformaciones salvo el 8, que presentaba bastantes números negativos y lo acotamos por $Q1-1.5IQR$ en vez de por 0. Aquí todos tienen una gran correlación con algún atributo, como no sabía con cuales quedarme, opté por quedarme con dos, y por elegir alguna, decidí mirar la forma de sus componentes principales. Ahí vi que todas estaban igual de representadas en la 1º componente. En la 2º componente dominó el Atributo 8, y en la 3º componente dominaron el atributo 40 y el 50 (un poco mas el 40). Por este motivo opté por quedarme con la variable 8 y la variable 40 (que de hecho también coinciden con el punto de menor correlación en la matriz de correlación.

```{r}
corrplot(cor(df_raw[,paste0("Attr",c(4,8,17,40,46,50))]), method = "circle", addCoef.col = "black")
prcomp(df_raw[,paste0("Attr",c(4,8,17,40,46,50))],scale=T)
df_raw=subset(df_raw,select=-c(Attr4,Attr17,Attr46,Attr50))
```

-   9 y 36. Estas dos variables les aplicamos el tratamiento genérico. Estas dos variables son muy similares:
    -   Attr9 ventas / activo total

    -   Attr36 ventas totales / activo total

```{r}
corrplot(cor(df_raw[,paste0("Attr",c(9,36))]), method = "circle", addCoef.col = "black")
```

-   13,19,23,31,39,42,49,56. Estas variables se trataron simultáneamente debido a la presencia de NAs por la división por ventas. Al compartir ese denominador, no es sorprendente que estén altamente correlacionadas. Aunque todos están muy relacionados, podemos ver como dos bloques: 13,19,23,31 y 39,42,49; también está el 56 que no está tan correlacionado con los demás. De estos eligimos el atributo 13 y 49, que son los menos relacionados con los de fuera de su grupo (su correlacion es solo de 0.53).

```{r}
corrplot(cor(df_raw[,paste0("Attr",c(13,19,23,31,39,42,49,56))]), method = "circle", addCoef.col = "black")
df_raw=subset(df_raw,select=-c(Attr19,Attr23,Attr31,Attr39,Attr42))
```



-   12,16 y 26. Estas tres variables son (alguna forma de beneficio)/(algún tipo de pasivo), por lo que no es sorprendente esta gran correlación que presentan, siendo el 16 y el 26 casi idénticos. Debido a su gran parecido nos quedaremos con el atributo 16, que tiene los beneficios brutos+amortizaciones (que lo he decidido preferir por tener el +amortizaciones, y ser el bruto el bruto (generalmente se habla de los sueldos en bruto)).
```{r}
corrplot(cor(df_raw[,paste0("Attr",c(12,16,26))]), method = "circle", addCoef.col = "black")
df_raw=subset(df_raw,select=-c(Attr12,Attr16))
```

-   1,7,11,14,18,22, 24,35 y 48. Salvo el 24,todos fueron tratados de forma genérica, y son todos de la forma: beneficio/activo. Aquí rápidamente vemos que están altísimamente correlacionadoes los atributos 1,7,11,14,18 y 22. Para quedarnos de estos 6 el que menos correlación tenga con los otros 3, nos quedaremos con el atributo 1 (también consideramos el 18, pero no vimos ningún motivo para elegir uno sobre el otro).

```{r}
corrplot(cor(df_raw[,paste0("Attr",c(1,7,11,14,18,22, 24,35,48))]), method = "circle", addCoef.col = "black")
df_raw=subset(df_raw,select=-c(Attr7,Attr11,Attr14,Attr18,Attr22))
```

Tras todo esto, hemos reducido nuestro dataframe de 65 atributos a 43 atributos. 

```{r}
saveRDS(df_raw,paste0(root.dir,"Datasets_procesados/1_2_Corrs_eliminadas"))
```


## Eliminación recursiva de variables

Hasta ahora hemos realizado un procesado básico de nuestros datos y eliminado variables altamente correladas. Ahora procederemos a elegir las variables que resultan más útiles para nuestro modelo. La base de datos está desbalanceada, por lo que no podemos usar el accuracy para elegir el mejor modelo (que es lo que queremos hacer con la función rfe). Para solucionar esto, haremos primero un undersampling, para que así la métrica del accuracy sea buena. 

Hemos elegido undersampling (disminuir la base de datos) frente al oversampling (aumentar la base de datos repitiendo filas (u otro método)) debido a que si hacemos un oversampling, luego especificarle los datasets específicos a la función rfe es mas dificil, y si no tienes cuidado con la función puedes acabar teniendo leakage del oversampling. Esto no sucede al hacer undersampling porque no has creado datos nuevos, por lo que puedes usar los defauts del algoritmo. Además, al hacer undersampling es más fácil correr este algoritmo (que es bastante lento). Nótese que así, nos quedamos con unas 1000 filas (que es lo malo de hacer undersampling vs oversampling que acabarías con unas 16k filas). Este númeo de filas es aproximadamente 20 veces el número de columnas, por lo que nos servirá adecuadamente como referencia para la selección de columnas (para un buen modelo se necesitan al menos 10 veces el número de columnas y se recomiendan en torno a 100 veces) [https://graphite-note.com/how-much-data-is-needed-for-machine-learning#:~:text=Generally%20speaking%2C%20the%20rule%20of,100%20rows%20for%20optimal%20results]. 
...
for(i in 1:20) seeds[[i]] <- sample.int(1000, length(subsets) + 1)
seeds[[21]] <- sample.int(1000, 1)


df_raw[,"class"]=factor(df_raw[,"class"])
#Por lo pequeño que es el dataset, es fácil hacerle un cross-val con muchos folds
#Así que aprovechamos y lo hacemos de 20 
...
df_down_sampled <- downSample(x = df_raw[,-ncol(df_raw)],
                               y = df_raw$class,
                               yname = "class")

rf.rfe <- rfe(class~., data=df_down_sampled,
              sizes=subsets, 
              rfeControl=ctrl.rfe)
...
```
```{r}
saveRDS(df_raw[,c(rf.rfe$optVariables,"class")],paste0(root.dir,"Datasets_procesados/1_3_RFE_skinny"))
```

Las variables que ha elegido rfe (buscando el clasificador random forest df_raw[,c(rf.rfe$optVariables,"class")]orest con mayor accuracy) son 1,2,5,6,8,13,21,24,25,26,35,40. Nótese que el SD de los modelos son muy altos (0.05 de un accuracy de 0.75), por lo que en cierto todos los modelos se podrían considerar equivalentes (y uno podría intentar quedarse con tan solo 5 variables).

```{r}
rf.rfe$optVariables
```

A continuación, miraremos cuales son las variables que selecciona un algoritmo genético que entrena poblaciones de Random Forests. Como nota, los algoritmos genéticos lo que hacen es entrenar n algoritmos (poblaciones de tamaño n), y hace que las características de los mejores sirvan para entrenar las siguientes generaciones, de manera que en cada iteración siempre se mejoran (o se mantiene igual) lo que se esté optimizando (en nuestro caso es una variable de fitness interna del algoritmo). En la lectura de la comparativa de las poblaciones, podemos ver que pone un porcentaje, esto es una comparativa del mejor de esa población con el mejor actual usando la medida de Jaccard. Podemos ver que este valor cuando nuestro modelo ya se ha estabilizado, se mantiene en torno a 90%, lo cual debería ser suficiente variación para que haya una mejora (en caso de que hubiera).

```{r}
ctrl.ga <- gafsControl(functions = rfGA, 
                           method ="cv",number = 5, 
                           returnResamp="all", 
                           allowParallel = TRUE,
                           verbose = TRUE)
...
set.seed(342)
system.time(rf.ga <- gafs(x = df_down_sampled[,-ncol(df_down_sampled)],
                               y = df_down_sampled$class,
                            iters = 50,
                            popSize = 10,
                            gafsControl = ctrl.ga))
...
rf.ga
```
A continuación mostraremos las variables que son consideradas óptimas para nuestro modelo. Así, podemos ver que hemos obtenido 31 variables (de 43), que son muchas, pero sorprendentemente no contienen todas las dadas por el algoritmo de rfe (faltan 25,26,40). Guardaremos esto como un dataset aparte del dado por rfe, y entrenaremos con este; sin embargo, cuando tengamos nuestro modelo final trataremos de entrenar con el de pocas variables para ver si se mantiene igual (en cuyo caso nos quedaríamos con el de menos variables).
```{r}
rf.ga$optVariables
```

```{r}
saveRDS(df_raw[,c(rf.ga$optVariables,"class")],paste0(root.dir,"Datasets_procesados/1_3_RFE_beefy"))
```


# Conclusiones
A lo largo de este documento le hemos quitado los valores faltantes y aplicado transformaciones a cada una de nuestras variables para preparar nuestra base de datos para su uso en modelos. Además, hemos hecho un análisis adicional del número de las variables mas relevantes para la predicción de nuestra clase, para así reducir la dimensionalidad de nuestra base de datos y hacer modelos mas robustos.