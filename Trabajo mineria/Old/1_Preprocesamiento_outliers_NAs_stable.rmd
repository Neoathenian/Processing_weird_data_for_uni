...
```{r}
df_raw=readRDS(df_raw,"1_Procesado_outliers_NAs.Rds")
```
...
-   28,53 y 54. Estas tres variables se procesaron a la vez; en este procesamiento también estaba el Attr64 pues las 4 variables tienen el mismo denominador, pero el Attr64 era la única que siempre era positiva, y por lo que se ve no está tan correlada, pero si lo está bastante (correlación 0.5-0.6 con cada una de las variables).

```{r}
corrplot(cor(df_raw[,paste0("Attr",c(28,53,54,64))]), method = "circle", addCoef.col = "black")
```

-   2,3,10,25,38 y 51. Estas variables fueron preprocesadas a la vez en el tratamiento de outliers restantes, aplicándoles transformaciones genéricas. Su alta correlación es probablemente debido a que comparten el denominador "activo total".
...
-   4,8,17,40,46 y 50. Todos estos atributos los tratamos a la vez, y tuvieron exactamente las mismas transformaciones salvo el 8, que presentaba bastantes números negativos y lo acotamos por $Q1-1.5IQR$ en vez de por 0.
